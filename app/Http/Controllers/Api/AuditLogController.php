    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            try {
                ...
                \Log::error('Audit logs - students query failed: ' . $e->getMessage());
            }
            ...
            try {
                $recentTeachers = DB::table('teachers')
                    ...
                    ->limit(3)
                    ->get();
                $logs = $logs->merge($recentTeachers);
            } catch (\Exception $e) {
                \Log::error('Audit logs - teachers query failed: ' . $e->getMessage());
            }
            ...
            try {
                $recentEnrollments = DB::table('student_courses')
                    ...
                    ->limit(5)
                    ->get();
                $logs = $logs->merge($recentEnrollments);
            } catch (\Exception $e) {
                \Log::error('Audit logs - enrollments query failed: ' . $e->getMessage());
            }
            ...
            try {
                $recentAttendance = DB::table('attendances')
                    ...
                    ->limit(5)
                    ->get();
                $logs = $logs->merge($recentAttendance);
            } catch (\Exception $e) {
                \Log::error('Audit logs - attendance query failed: ' . $e->getMessage());
            }
            ...
            try {
                $recentGrades = DB::table('grades')
                    ...
                    ->limit(5)
                    ->get();
                $logs = $logs->merge($recentGrades);
            } catch (\Exception $e) {
                \Log::error('Audit logs - grades query failed: ' . $e->getMessage());
            }
